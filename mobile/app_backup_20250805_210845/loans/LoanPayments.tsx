      ...
      <View className="px-3 pt-5">
        ...
        <View className="bg-white rounded-xl shadow p-4 border border-gray-100">
          ...
          {currentLoan?.payments?.map((item, idx) => (
            <View key={idx} className="flex-row justify-between items-center mb-2">
              ...
              {item.status === 'PAID' ? (
                ...
                <Text className="text-xs bg-orange-400 text-white px-2 py-1 rounded-full font-semibold">Pending</Text>
              ) : (
                <Text className="text-xs bg-blue-400 text-white px-2 py-1 rounded-full font-semibold">Scheduled</Text>
              )}
            </View>
          ))}
          ...
          <View className="flex-row justify-center mt-2">
            ...
            onPress={() => router.push('/loans/MakePayment')}>
