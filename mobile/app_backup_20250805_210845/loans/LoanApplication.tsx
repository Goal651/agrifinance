...
import { DocumentUpload, FinancialInfo, LoanRequest, PersonalInfo } from '@/types';
...
export default function LoanApplication() {
  ...
  const { currentLoanProduct } = useLoan()
  ...
  const handleSubmit = async () => {
    if (!purpose || !currentLoanProduct || !personalInfo || !financialInfo || !documents) return;

    const loanRequest: LoanRequest = {
      purpose,
      details: currentLoanProduct,
      personalInfo,
      financialInfo,
      documents
    };
  ...
  if (!currentLoanProduct) return (
    <View>
      <Text>There have been an error</Text>
    </View>
  )
  // ===== RENDER FUNCTIONS =====
    ...
    <ScrollView className="flex-1 bg-gray-50 px-8" showsVerticalScrollIndicator={false}>
      ...
      <LoanCard title="Loan Product Details">
        ...
        <LoanInputField
          label="Loan Amount"
          placeholder=""
          value={`$${Number(currentLoanProduct.amount).toLocaleString()}`}
          onChangeText={() => { }}
          icon="attach-money"
          readOnly={true}
        />
        <LoanInputField
          label="Interest Rate"
          placeholder=""
          value={`${currentLoanProduct.interest}%`}
          onChangeText={() => { }}
          icon="percent"
          readOnly={true}
        />
        <LoanInputField
          label="Loan Term"
          placeholder=""
          value={`${currentLoanProduct.termType === 'YEARS' ? Number(currentLoanProduct.term / 12) : currentLoanProduct.term} ${currentLoanProduct.termType}`}
          onChangeText={() => { }}
          icon="schedule"
          readOnly={true}
        />
        <LoanInputField
          ...
          placeholder=""
          value={`$${Number(calculateMonthlyPayment(currentLoanProduct.amount, currentLoanProduct.interest, currentLoanProduct.term)).toLocaleString()}`}
          onChangeText={() => { }}
          icon="payment"
          readOnly={true}
        />
    ...
    <ScrollView className="flex-1 bg-gray-50 px-8" showsVerticalScrollIndicator={false}>
      ...
      <LoanCard title="Loan Summary">
        <LoanInputField
          label="Loan Term"
          placeholder=""
          value={Number(currentLoanProduct.term) >= 12 ? `${Math.round(Number(currentLoanProduct.term) / 12)} years` : `${currentLoanProduct.term} months`}
          onChangeText={() => { }}
          icon="schedule"
          readOnly={true}
        />
        ...
        <LoanInputField
          ...
          placeholder=""
          value={`$${((Number(calculateMonthlyPayment(currentLoanProduct.amount, currentLoanProduct.interest, currentLoanProduct.term)) * Number(currentLoanProduct.term)) - Number(currentLoanProduct.amount)).toFixed(2)}`}
          onChangeText={() => { }}
          icon="trending-up"
          readOnly={true}
        />
        <LoanInputField
          ...
          placeholder=""
          value={`$${(Number(calculateMonthlyPayment(currentLoanProduct.amount, currentLoanProduct.interest, currentLoanProduct.term)) * Number(currentLoanProduct.term)).toFixed(2)}`}
          onChangeText={() => { }}
          icon="account-balance-wallet"
          readOnly={true}
        />
      ...
      <LoanCard title="Loan Details">
        <View className="space-y-4">
          <View className="flex-row justify-between">
            ...
            <Text className="font-semibold text-base">{currentLoanProduct.name}</Text>
          </View>
          <View className="flex-row justify-between">
            <Text className="text-gray-500 text-base">Amount:</Text>
            <Text className="font-semibold text-base">${Number(currentLoanProduct.amount).toLocaleString()}</Text>
          </View>
          <View className="flex-row justify-between">
            <Text className="text-gray-500 text-base">Interest Rate:</Text>
            <Text className="font-semibold text-base">{currentLoanProduct.interest}%</Text>
          </View>
          <View className="flex-row justify-between">
            <Text className="text-gray-500 text-base">Term:</Text>
            <Text className="font-semibold text-base">
              {Number(currentLoanProduct.term) >= 12 ? `${Math.round(Number(currentLoanProduct.term) / 12)} years` : `${currentLoanProduct.term} months`}
            </Text>
          </View>
          <View className="flex-row justify-between">
            <Text className="text-gray-500 text-base">Monthly Payment:</Text>
            <Text className="font-semibold text-base">${Number(calculateMonthlyPayment(currentLoanProduct.amount, currentLoanProduct.interest, currentLoanProduct.term)).toLocaleString()}</Text>
          </View>
          <View className="flex-row justify-between">
