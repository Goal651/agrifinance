...
export default function LoanAnalyticsScreen() {
  ...
  const stats = useMemo(() => ({
    ...
    remainingInterest: loanAnalytics
      ? loanAnalytics.loanBreakdown.reduce(
          (sum: number, l: { remainingAmount: number; interest: number; amount: number }) =>
            sum + (l.remainingAmount * l.interest) / (l.amount || 1),
          0
        )
      : 0,
    remainingPrincipal:
      loanAnalytics?.loanBreakdown.reduce(
        (sum: number, l: { remainingAmount: number }) => sum + l.remainingAmount,
        0
      ) || 0,
  }), [loanAnalytics]);
  ...
  const total =
    ...
    stats.remainingPrincipal;
  const principalPercent = total
    ? ((stats.principalPaid + stats.remainingPrincipal) / total) * 100
    : 0;
  ...
  const balance = useMemo(() => {
    ...
    return loanAnalytics.paymentHistory.map((p: { amount: number }) => {
      running -= p.amount;
      return running;
    });
  }, [loanAnalytics]);
  ...
  const filteredMonths =
    loanAnalytics?.paymentHistory?.map((p: { amount: number }, i: number) =>
      i % 5 === 0 ? `#${i + 1}` : ''
    ) || [];
  ...
  const points = balance.map((val: number, i: number) => {
  ...
  const areaPath = `${points
    ...
    .join(' ')} L${points[points.length - 1]?.x || 30},${chartHeight - 30} L${points[0]?.x || 30},${
    chartHeight - 30
  } Z`;
        ...
        <View className="bg-white rounded-xl shadow p-4 mb-4 border border-gray-100">
          ...
          <View className="bg-gray-50 rounded-xl p-2 items-center">
            <Svg width={chartWidth} height={chartHeight}>
              ...
              <SvgText x={65} y={36} fontSize="14" fill="#222">
                Loan Balance
              </SvgText>
              {/* Y axis labels */}
              {[0, 1, 2, 3, 4].map((i) => (
          ...
          <View className="flex-row items-center justify-between mt-2">
            ...
            <View className="items-center">
              <Svg width={110} height={110}>
                ...
                <SvgText x={55} y={72} fontSize="12" fill="#888" textAnchor="middle">
                  Total
                </SvgText>
              </Svg>
            </View>
